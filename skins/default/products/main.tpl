...
while($row2 = $res->fetch_assoc()){?>
	<span class="prod-cat"><a href="/products?cat=<?php echo (int)$row2['id'];?>"><?php echo hc($row2['name']);?></a></span><?php
}?>
	...
	while($row = mysqli_fetch_assoc($products)) {?>
		...
		<div class="prod-info">
			<div><?php echo '<span class="prod-parameter">Категория: </span>'.hc($row['category']).'<br><span class="prod-parameter">Название: </span>'.hc($row['name']);
				if(isset($_GET['id'])){echo '<br><span class="prod-parameter">Код товара: </span>'.(int)$row['code'].'<br><span class="prod-parameter">Описание: </span><br>'.nl2br(hc($row['description']));}
